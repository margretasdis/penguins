...
#Plot the data with our model lines included to aid with the interpretation

r Simple plot for aiding the interpretation of the statistical test}

plot_beak_boxplot <- ggplot(penguins_culmen, aes(x = body_mass_g, y = culmen_length_mm, colour = species, shape = species), alpha = 0.05) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE) +
  theme_bw() +
...
#colour intervals, remove latin names?
